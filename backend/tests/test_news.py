...
def test_news_loader_rejects_insecure_or_invalid_youtube_urls(tmp_path: Path) -> None:
    ...
    path.write_text(json.dumps([
        {"id": "bad-http", "number": 1, "name": "Bad", "url": "http://www.youtube.com/@dwnews/live", "enabled": True}
    ]), encoding="utf-8")
    with pytest.raises(ValidationError):
        load_news_channels(path)

    path_sub = tmp_path / "subdomain.json"
    path_sub.write_text(json.dumps([
        {"id": "bad-sub", "number": 1, "name": "Bad", "url": "https://m.youtube.com/@dwnews/live", "enabled": True}
    ]), encoding="utf-8")
    with pytest.raises(ValidationError):
...
def test_news_loader_rejects_duplicate_ids_or_numbers(tmp_path: Path) -> None:
    ...
    path.write_text(json.dumps([
        {"id": "dup", "number": 1, "name": "One", "url": "https://www.youtube.com/@dwnews/live", "enabled": True},
        {"id": "dup", "number": 2, "name": "Two", "url": "https://www.youtube.com/@pbsnewshour/live", "enabled": True},
    ]), encoding="utf-8")
    with pytest.raises(ValueError, match="IDs must be unique"):
        load_news_channels(path)

    path_num = tmp_path / "dup_num.json"
    path_num.write_text(json.dumps([
        {"id": "one", "number": 1, "name": "One", "url": "https://www.youtube.com/@dwnews/live", "enabled": True},
        {"id": "two", "number": 1, "name": "Two", "url": "https://www.youtube.com/@pbsnewshour/live", "enabled": True},
    ]), encoding="utf-8")
